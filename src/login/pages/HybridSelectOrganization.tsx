...
export default function HybridSelectOrganization(props: PageProps<Extract<KcContext, { pageId: "select-organization.ftl" }>, I18n>) {
    ...
    return (
        <Template kcContext={kcContext} i18n={i18n} doUseDefaultCss={doUseDefaultCss} classes={classes} headerNode={msg("organization.select")}>
            <form action={url.loginAction} method="post" className="space-y-6" onSubmit={handleSubmit}>
                {/* Intro */}
                <div className="p-4 rounded-lg bg-blue-50 border border-blue-200">
                    <p className="text-sm text-blue-900 font-medium">{msg("organization.select")}</p>
                </div>
                    ...
                    {user?.organizations && user.organizations.length > 0 ? (
                        user.organizations.map(org => (
                            ...
                            >
                                ...
                                />
                                <div className="ml-3">
                                    <p className="font-semibold text-gray-900">{org.name || org.alias}</p>
                                    <p className="text-xs text-gray-500 mt-1">{org.alias}</p>
                                </div>
                            </label>
                        ))
                    ) : (
                ...
                {selectedOrg && <input type="hidden" name="organization" value={selectedOrg} />}
